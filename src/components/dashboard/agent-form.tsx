...
import { Loader2, Wheat, TrendingUp, CloudSun, ClipboardList, Tractor, Pen, Puzzle } from "lucide-react";
...
const presetRingColor: Record<PersonalityPreset, string> = {
...
export function AgentForm({
  ...
  // Auto-set skills when preset changes (only for new agents)
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {PERSONALITY_PRESETS.map((preset) => {
              ...
              const isSelected = personalityPreset === preset;
              return (
                <button
                  key={preset}
                  type="button"
        ...
        <div>
          ...
          <div className="space-y-3">
            {BUILT_IN_SKILLS.map((skill) => {
              ...
              const checked = (skills as string[]).includes(skill);
              return (
            ...
            {customSkills.length > 0 && (
              <>
                ...
                {customSkills.map((cs) => {
                  const checked = (skills as string[]).includes(cs.slug);
                  return (
        ...
        <div>
          ...
          <div className="space-y-3">
            {AVAILABLE_CRON_JOBS.map((cron) => {
              ...
              const skillDisabled =
                !(skills as string[]).includes(info.requiredSkill) ||
                isGloballyDisabled(info.requiredSkill);
              const checked = !!(cronJobs as Record<string, boolean>)[cron];
              return (

...
type EdgeBuilderAction =
  | { type: "set-source"; value: string }
  | { type: "set-target"; value: string }
  | { type: "set-when"; value: WorkflowEdgeCondition }
  | { type: "set-error"; value: string | null };

const INITIAL_EDGE_BUILDER_STATE: EdgeBuilderState = {
  source: "",
  target: "",
  when: "always",
  error: null,
...
function edgeBuilderReducer(state: EdgeBuilderState, action: EdgeBuilderAction): EdgeBuilderState {
  switch (action.type) {
    case "set-source":
      return {
        ...state,
...
function useWorkflowCanvasContext(): WorkflowCanvasContextValue {
  const context = use(WorkflowCanvasContext);
  if (!context) {
    throw new Error("Workflow canvas subcomponents must be used within <WorkflowCanvas>");
  }
...
const NodeLayer = memo(
  function NodeLayer({ nodes }: NodeLayerProps) {
    const { selectedNodeId, onSelectNode } = useWorkflowCanvasContext();
...
interface WorkflowCanvasToolbarProps {
  nodeSelectOptions: Array<{ id: string; label: string }>;
  connectSource: string;
  connectTarget: string;
  effectiveConnectWhen: WorkflowEdgeCondition;
  whenOptions: WorkflowEdgeCondition[];
  connectError: string | null;
  onConnectSourceChange: (value: string) => void;
  onConnectTargetChange: (value: string) => void;
  onConnectWhenChange: (value: WorkflowEdgeCondition) => void;
  onCreateEdge: () => void;
      ...
      <div className="flex flex-wrap items-end gap-2 border-b border-border px-4 py-3">
        <div className="min-w-[170px] flex-1">
          ...
          <select
            ...
            value={connectSource}
            onChange={(event) => {
              onConnectSourceChange(event.target.value);
            }}
        ...
        <div className="min-w-[170px] flex-1">
          ...
          <select
            ...
            value={connectTarget}
            onChange={(event) => {
              onConnectTargetChange(event.target.value);
            }}
            className={`w-full rounded-lg border border-border bg-bg-dark px-2.5 py-2 text-sm text-text-primary ${FOCUS_RING_CLASS}`}
          >
            <option value="">Select node</option>
        ...
        <div className="min-w-[125px]">
          ...
          <select
            ...
            onChange={(event) => {
              onConnectWhenChange(event.target.value as WorkflowEdgeCondition);
            }}
            className={`w-full rounded-lg border border-border bg-bg-dark px-2.5 py-2 text-sm text-text-primary ${FOCUS_RING_CLASS}`}
...
interface WorkflowCanvasEdgeListProps {
  edges: WorkflowEdge[];
  hiddenEdgeListCount: number;
}

function WorkflowCanvasEdgeList({ edges, hiddenEdgeListCount }: WorkflowCanvasEdgeListProps) {
  const { nodeMap, onDeleteEdge } = useWorkflowCanvasContext();

  return (
    <div className="border-t border-border px-4 py-3">
      <div className="mb-2 flex items-center justify-between">
        <h5 className="text-xs font-semibold uppercase tracking-wide text-text-dim">
          Edges ({edges.length + hiddenEdgeListCount})
        </h5>
        ...
        <div className="space-y-1.5">
          ...
          {hiddenEdgeListCount > 0 && (
            <p className="text-xs text-text-dim">
              Showing first {edges.length} edges. {hiddenEdgeListCount} additional edges are
...
export function WorkflowCanvas({
  ...
  const [edgeBuilderState, dispatchEdgeBuilder] = useReducer(
    edgeBuilderReducer,
    INITIAL_EDGE_BUILDER_STATE
  );
  ...
  const connectSource = edgeBuilderState.source;
  const connectTarget = edgeBuilderState.target;
  const connectWhen = edgeBuilderState.when;
  const connectError = edgeBuilderState.error;

  const setConnectSource = useCallback((value: string) => {
    dispatchEdgeBuilder({ type: "set-source", value });
  }, []);
  const setConnectTarget = useCallback((value: string) => {
    dispatchEdgeBuilder({ type: "set-target", value });
  }, []);
  const setConnectWhen = useCallback((value: WorkflowEdgeCondition) => {
    dispatchEdgeBuilder({ type: "set-when", value });
  }, []);
  const setConnectError = useCallback((value: string | null) => {
    dispatchEdgeBuilder({ type: "set-error", value });
  }, []);
  ...
  const contextValue = useMemo(
    ...
    [selectedNodeId, onSelectNode, onDeleteEdge, nodeMap]
  );

  return (
    <WorkflowCanvasContext value={contextValue}>
      <section className="rounded-2xl border border-border bg-bg-card/70">
        <WorkflowCanvasToolbar
          nodeSelectOptions={nodeSelectOptions}
          connectSource={connectSource}
          connectTarget={connectTarget}
          effectiveConnectWhen={effectiveConnectWhen}
          whenOptions={whenOptions}
          connectError={connectError}
          onConnectSourceChange={setConnectSource}
          onConnectTargetChange={setConnectTarget}
          onConnectWhenChange={setConnectWhen}
          onCreateEdge={handleCreateEdge}
          canCreateEdge={graph.nodes.length >= 2}
          zoom={zoom}
          onZoomOut={zoomOut}
          onZoomIn={zoomIn}
          onFitView={fitView}
        ...
        <WorkflowCanvasEdgeList
          edges={listedEdges}
          hiddenEdgeListCount={hiddenEdgeListCount}
        />
      </section>

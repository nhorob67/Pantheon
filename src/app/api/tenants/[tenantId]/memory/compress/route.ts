...
const tenantMemoryCompressRouteParamsSchema = z.object({
  tenantId: z.uuid(),
});
...
export async function POST(
  ...
  const parsedParams = await parseTenantRouteParams({
    request,
    params,
    schema: tenantMemoryCompressRouteParamsSchema,
  ...
  return runTenantRoute(
    request,
    {
      ...
      fallbackErrorMessage: "Failed to queue tenant memory compression",
    },
    async (state) => {
      ...
      if (mapping.ambiguous) {
        responseBody.warning =
          "Multiple active legacy instance mappings detected. Using most recent mapping for memory compression.";
      }

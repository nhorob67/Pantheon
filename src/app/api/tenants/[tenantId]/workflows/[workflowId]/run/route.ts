...
export async function POST(
  ...
  return runTenantRoute(
    request,
    {
      ...
      requiredGate: "writes",
      fallbackErrorMessage: "Failed to create workflow run",
    },
    async (state) => {
      ...
      const { data: publishedVersion, error: publishedVersionError } =
        await state.admin
      ...
      const { data: runRow, error: runError } = await state.admin
        .from("workflow_runs")
        .insert({
      ...
      if (requiresApproval) {
        ...
        if (approvalInsertError) {
          ...
          const runMetadata = normalizeObject(
          ...
          await state.admin
            ...
            .update({
              status: "failed",
              error_message:
                "Failed to initialize workflow approvals for this run.",

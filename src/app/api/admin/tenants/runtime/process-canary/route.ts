...
import {
  createDiscordCanaryDispatchWorker,
  noOpDiscordCanaryWorker,
} from "@/lib/runtime/tenant-runtime-worker";
import { TENANT_DISCORD_CANARY_DISPATCH_FLAG_KEY } from "@/lib/runtime/tenant-runtime-gates";
...
export async function POST(request: Request) {
  ...
  const workerId = url.searchParams.get("worker_id") || "tenant-runtime-canary-worker";

  try {
    const admin = createAdminClient();
    const dispatchWorker = createDiscordCanaryDispatchWorker(admin);
    ...
    for (const claim of claims) {
      try {
        if (!dispatchFlagCache.has(claim.run.customer_id)) {
          const enabled = await resolveCustomerFeatureFlag(
            admin,
            claim.run.customer_id,
            TENANT_DISCORD_CANARY_DISPATCH_FLAG_KEY
          );
          dispatchFlagCache.set(claim.run.customer_id, enabled);
        }

        const dispatchEnabled = dispatchFlagCache.get(claim.run.customer_id) === true;
        const worker = dispatchEnabled ? dispatchWorker : noOpDiscordCanaryWorker;
        ...
        try {
          const failedRun = await transitionTenantRuntimeRun(admin, claim.run, "fail", {
            workerId,
            errorMessage: safeErrorMessage(error, "No-op canary worker failed"),
            metadataPatch: {
          ...
          failed += 1;
          deadLettered += 1;
          // Best-effort failure transition for scaffolding route.
        }
      }
    }
    ...
    return NextResponse.json(
      {
        error: safeErrorMessage(error, "Failed to process tenant runtime canary queue"),
      },
      { status: 500 }
    );
  }

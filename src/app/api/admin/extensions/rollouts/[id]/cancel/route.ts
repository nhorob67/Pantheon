...
import {
  RolloutInputError,
  markPendingRolloutTargetsSkipped,
...
export async function POST(
  ...
  try {
    ...
    await markPendingRolloutTargetsSkipped(admin, rollout.id);

    auditLog({
      action: "extension.rollout.cancel",
      actor: user.email || user.id,
    ...
    return NextResponse.json(
      { error: safeErrorMessage(error, "Failed to cancel extension rollout") },
      { status: 500 }
    );
  }
}
